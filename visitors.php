...
function fetchUrl($url) {
    ...
    if (ini_get('allow_url_fopen')) {
        $ctx = stream_context_create([
            'http' => ['timeout' => 5, 'header' => "User-Agent: VisitorCounter/1.0\r\n"]
        ]);
...
function geoLookup($ip) {
    ...
    if ($json) {
        ...
        if (isset($d['status']) && $d['status'] === 'success') {
            return ['code' => strtolower($d['countryCode']), 'name' => $d['country'], 'city' => $d['city'] ?? '', 'region' => $d['regionName'] ?? ''];
        }
    }
    ...
    if ($json2) {
        ...
        if (isset($d2['country_code'])) {
            return ['code' => strtolower($d2['country_code']), 'name' => $d2['country'] ?? 'Unknown', 'city' => $d2['city'] ?? '', 'region' => $d2['region'] ?? ''];
        }
    }
    ...
    if ($json3) {
        ...
        if (isset($d3['country_code']) && !isset($d3['error'])) {
            return ['code' => strtolower($d3['country_code']), 'name' => $d3['country_name'] ?? 'Unknown', 'city' => $d3['city'] ?? '', 'region' => $d3['region'] ?? ''];
        }
    }
    return null;
}
...
try {
    ...
    if (!file_exists($dataFile)) {
        $initial = ['totalCount' => 0, 'todayCount' => 0, 'todayDate' => date('Y-m-d'), 'monthCount' => 0, 'monthDate' => date('Y-m'), 'countries' => [], 'recentVisitors' => [], 'ipCache' => []];
    ...
    if (!$fp) {
        ob_end_clean();
        echo json_encode(['success' => false, 'error' => 'Cannot open data file']);
        exit;
    }
    ...
    if (isset($_GET['visit'])) {
        ...
        if (isset($data['ipCache'][$ip]) && (time() - $data['ipCache'][$ip]['t']) < 86400) {
            ...
            if ($visitorGeo) $data['ipCache'][$ip] = ['geo' => $visitorGeo, 't' => time()];
        }
        ...
        if (isset($_GET['s'])) {
            ...
            foreach ($data['recentVisitors'] as &$v) {
                if ($v['sid'] === $sid) { $v['ts'] = $now; $found = true; break; }
            }
            unset($v);
